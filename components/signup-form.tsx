...
export function SignupForm() {
  ...
  async function handleInitialSubmit(e: React.FormEvent) {
    ...
    if (stage === "details") {
      if (!agreed) {
        showCustomModal("Agreement Required", "You must agree to the Terms of Service and Privacy Policy.");
        return;
      }
      setBusy(true);
      try {
      ...
      try {
        ...
        showCustomModal("Success!", "Your account has been created and verified successfully.", "success");
        setTimeout(() => {
  ...
  return (
    <div className="min-h-screen text-zinc-100 flex flex-col justify-between overflow-x-hidden relative w-full bg-[#121214]">
      ...
      <main className="min-h-screen w-full flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-[45%] bg-[#121214] flex flex-col justify-between px-6 py-6 sm:p-12 md:p-16 min-h-screen lg:min-h-0 border-r border-zinc-900 z-10">
          {/* Top Branding */}
          <div>
          ...
          {/* Center Form */}
          <div className="my-auto py-4 sm:py-12 max-w-sm w-full mx-auto">
            <h1 className="text-3xl sm:text-4xl font-bold text-white tracking-tight leading-none">Sign Up</h1>
            <p className="text-zinc-400 mt-2 text-sm sm:text-base font-normal">Create your student account</p>

            <form onSubmit={handleInitialSubmit} className="mt-6 sm:mt-8 space-y-4 sm:space-y-5">
              {stage === "details" ? (
                <div className="space-y-4 sm:space-y-5">
                  {/* Full Name Input */}
                  <div className="relative group">
                    <label htmlFor="name" className="block text-xs font-semibold uppercase tracking-widest text-[#8b5cf6]">Full Name</label>
                    <input 
                      id="name" 
                      type="text" 
                      required
                      ...
                      className="w-full bg-transparent border-b border-zinc-800 focus:border-[#8b5cf6] text-zinc-100 py-2 sm:py-3 outline-none text-[14px] sm:text-[15px] transition-colors placeholder-zinc-700" 
                  ...
                  {/* Username Input */}
                  <div className="relative group">
                    <label htmlFor="username" className="block text-xs font-semibold uppercase tracking-widest text-[#8b5cf6]">Username</label>
                    <input 
                      id="username" 
                      type="text" 
                      required
                  ...
                  {/* Email ID Input */}
                  <div className="relative group">
                    <label htmlFor="email" className="block text-xs font-semibold uppercase tracking-widest text-[#8b5cf6]">Email ID</label>
                    <input 
                      id="email" 
                      type="email" 
                      required
                  ...
                  {/* Agreement Checkbox */}
                  <label className="flex items-start gap-3 mt-3 sm:mt-4 text-xs text-zinc-500 select-none cursor-pointer group">
                    <input 
                      type="checkbox" 
                      required
                      checked={agreed}
                      onChange={(e) => setAgreed(e.target.checked)}
                      className="mt-0.5 h-4 w-4 rounded border-zinc-800 bg-transparent text-[#8b5cf6] focus:ring-0 focus:ring-offset-0 transition-colors"
                    />
                    <span className="leading-tight text-zinc-400 group-hover:text-zinc-300 transition-colors">
                      I agree to the <a href="#" className="hover:underline font-medium text-[#09eeb5e3]">Terms of Service</a> and <a href="#" className="hover:underline font-medium text-[#09eeb5e3]">Privacy Policy</a>
                    </span>
                  </label>
                </div>
              ) : (
              ...
              <div className="pt-3 sm:pt-4">
                ...
                >
                  {stage === "details" ? (
                    ...
                    <>
                      <span>{busy ? "Verifying..." : "Verify & Sign Up"}</span>
                      <span className={`material-symbols-outlined text-[18px] ${busy ? "animate-spin" : ""}`}>
                        {busy ? "sync" : "vpn_key"}
                      </span>
                    </>
          ...
          <div className="flex items-center justify-between gap-4 border-t border-zinc-900/60 pt-6">
            <span className="text-xs sm:text-sm text-zinc-500 font-normal">Already have an account?</span>
            <Link href="/login" className="inline-flex justify-center items-center px-6 py-3 rounded-full bg-[#242428] hover:bg-zinc-800 text-xs sm:text-sm font-semibold text-white transition-all duration-300 active:scale-95">
              Log in
            </Link>
          </div>
        </div>
        ...
        <div className="hidden lg:flex w-full lg:w-[55%] bg-[#8b5cf6] relative overflow-hidden flex-col justify-between p-16 text-white">
          ...
          <div className="absolute inset-0 pointer-events-none z-0">
            <svg className="absolute w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none" fill="none">
              ...
              <path d="M-15 50 C 15 65, 30 35, 65 60 C 85 75, 95 80, 115 95 L -15 95 Z" fill="white" fillOpacity="0.04" />
              <path d="M-30 30 C 20 15, 50 55, 110 20 L 110 -20 L -30 -20 Z" fill="black" fillOpacity="0.03" />
              <circle cx="20" cy="75" r="30" fill="white" fillOpacity="0.05" />
            </svg>
          </div>
          ...
          <div className="relative z-10 max-w-md">
            ...
            <p className="text-purple-100/75 mt-3 text-base font-normal tracking-wide">Register to get started</p>
          </div>
